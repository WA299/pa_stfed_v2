"""Validation-only runner for PUC-RSTAttn V2; never evaluates test labels."""
...
GRID_SHORT_NAMES = {"39_bus_semi_urban_reference_grid": "39bus", "50_bus_rural_reference_grid": "50bus", "56_bus_semi_urban_reference_grid": "56bus", "80_bus_rural_reference_grid": "80bus"}
...
def output_paths(grid_name: str, output_dir: Path) -> tuple[Path, Path]:
    ...
    stem = f"puc_rstattn_v2_{GRID_SHORT_NAMES[grid_name]}_{FEATURE_MODE}"
...
def _to_batch(dataset: ForecastWindowDataset, scaler: Any, indices: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    return (
        np.stack([scaler.transform(item["x"]) for item in windows]).astype(np.float32),
        np.stack([scaler.transform_target(item["y"]) for item in windows]).astype(np.float32),
    )
...
def report_metadata() -> dict[str, Any]:
    return {
        "model": "PUC-RSTAttn-V2",
        "implementation": "utility_anchored_hierarchical_residual_st_attention",
        ...
        "initial_spatial_attention": "normalized_positive_predictive_utility",
        "dynamic_attention_scale_initial": 0.0,
        "physical_bias_zero_initialized": True,
        "physical_relation_features": ["normalized_impedance_abs_distance", "normalized_hop_distance", "direct_physical_adjacency"],
...
def build_epoch_record(epoch: int, final_losses: list[float], anchor_losses: list[float], total_losses: list[float], validation: dict[str, Any], temporal_residual: dict[str, Any], gru_anchor: dict[str, Any]) -> dict[str, Any]:
    if not final_losses or not anchor_losses or not total_losses:
        raise ValueError("epoch loss lists must be non-empty")
    return {"epoch": int(epoch), "train_scaled_mae": float(np.mean(final_losses)), "train_anchor_scaled_mae": float(np.mean(anchor_losses)), "train_total_loss": float(np.mean(total_losses)), "validation": validation, "temporal_residual": temporal_residual, "gru_anchor": gru_anchor}


def _evaluate(model: Any, dataset: ForecastWindowDataset, scaler: Any, batch_size: int, device: Any, load_mask: Any) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, float]]:
    import torch
    final_predictions, temporal_predictions, anchor_predictions, actual = [], [], [], []
    diagnostics: dict[str, list[float]] = {"mean_temporal_gate": [], "mean_spatial_gate_active": [], "mean_abs_temporal_correction": [], "mean_abs_centered_spatial_residual": [], "mean_spatial_attention_entropy": [], "aggregate_preservation_max_abs_error": []}
    model.eval()
    with torch.no_grad():
        for indices in _batch_indices(len(dataset), batch_size):
            ...
            active_gate = details["spatial_gate"][:, model.load_bus_mask.to(device) & (details["reliability"][:, 2] > 0)]
    ...
    return evaluate_validation(actual_array, final_array, load_mask), evaluate_validation(actual_array, temporal_array, load_mask), evaluate_validation(actual_array, anchor_array, load_mask), {key: float(np.mean(values)) for key, values in diagnostics.items()}
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    model = PUCRSTAttnV2(grid.num_nodes, utility_graph.edge_index, utility_graph.relation_features[:, 1:], grid.load_bus_mask, utility_graph.relation_features[:, 0]).to(device)
    ...
    for epoch in range(1, args.max_epochs + 1):
        model.train(); final_losses, anchor_losses, total_losses = [], [], []
        for indices in _batch_indices(len(train_dataset), args.batch_size):
            x, y = _to_batch(train_dataset, scaler, indices); optimizer.zero_grad(set_to_none=True)
            final, _, details = model(torch.from_numpy(x).to(device), return_details=True); target = torch.from_numpy(y).to(device)
            final_loss = masked_scaled_mae(final, target, load_mask); anchor_loss = masked_scaled_mae(details["y_gru"], target, load_mask); total_loss = final_loss + ANCHOR_LOSS_WEIGHT * anchor_loss
            total_loss.backward(); optimizer.step(); final_losses.append(float(final_loss.detach())); anchor_losses.append(float(anchor_loss.detach())); total_losses.append(float(total_loss.detach()))
        ...
        if current < best_validation:
            best_validation, best_state, stale_epochs = current, {key: value.detach().cpu().clone() for key, value in model.state_dict().items()}, 0
        else:
            stale_epochs += 1
            if stale_epochs >= args.patience: break
    ...
    return {"config": config, "graph_diagnostics": utility_graph.diagnostics, "validation": {"final": final, "temporal_residual": temporal, "gru_anchor": anchor_metrics, "persistence_1h": persistence}, "validation_diagnostics": diagnostics, "summary": best_epoch_summary(history), "metric_units": {"mae": "P units", "rmse": "P units", "wape_pct": "%", "smape_pct": "%"}, "training_history": history, "test_evaluated": False}


def render_markdown(report: dict[str, Any]) -> str:
    lines = [f"# PUC-RSTAttn V2: {report['config']['grid_name']} / {FEATURE_MODE}", "", "Validation-only metrics; no test labels are accessed.", "", "| Method | Scope | MAE | RMSE | WAPE (%) | sMAPE (%) |", "| --- | --- | ---: | ---: | ---: | ---: |"]
    for method in ("final", "temporal_residual", "gru_anchor", "persistence_1h"):
        for scope in ("node_macro", "grid_aggregate"):
            values = report["validation"][method][scope]; lines.append(f"| {method} | {scope} | {values['mae']:.6g} | {values['rmse']:.6g} | {values['wape_pct']:.6g} | {values['smape_pct']:.6g} |")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--grid", choices=GRID_NAMES, default=GRID_NAMES[0]); parser.add_argument("--device", default="cpu"); parser.add_argument("--seed", type=int, default=DEFAULT_SEED); parser.add_argument("--max-epochs", type=int, default=DEFAULT_MAX_EPOCHS); parser.add_argument("--patience", type=int, default=DEFAULT_PATIENCE); parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE); parser.add_argument("--learning-rate", type=float, default=DEFAULT_LEARNING_RATE); parser.add_argument("--data-root", type=Path, default=REPO_ROOT.parent / "pa_stfed_data_v2" / "raw"); parser.add_argument("--mapping-json", type=Path, default=REPO_ROOT / "results" / "audits" / "v2_schema_mapping.json"); parser.add_argument("--output-dir", type=Path, default=REPO_ROOT / "results" / "centralized"); return parser


def main() -> None:
    args = build_parser().parse_args(); output_json, output_md = output_paths(args.grid, args.output_dir); report = run_training(args); output_json.parent.mkdir(parents=True, exist_ok=True); output_json.write_text(json.dumps(report, indent=2, ensure_ascii=True) + "\n", encoding="utf-8"); output_md.write_text(render_markdown(report) + "\n", encoding="utf-8"); print(f"wrote {output_json} and {output_md}")
