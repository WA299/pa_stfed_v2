"""Validation-only shared-GRU electrical-distance-biased attention runner."""
...
from code.models.electrical_attention import ElectricalAttentionBaseline
...
def _compare_electrical_persistence(electrical: dict[str, Any], persistence: dict[str, Any]) -> dict[str, dict[str, str]]:
    comparison: dict[str, dict[str, str]] = {}
    for scope in ("node_macro", "grid_aggregate"):
        comparison[scope] = {}
        for metric in REPORT_METRICS:
            electrical_value = float(electrical[scope][metric])
            persistence_value = float(persistence[scope][metric])
            if np.isclose(electrical_value, persistence_value, rtol=1e-12, atol=1e-12):
                relation = "equal"
            elif electrical_value < persistence_value:
                relation = "electrical_attention_lower"
            else:
                relation = "electrical_attention_higher"
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    distance_matrix = grid.distance_matrices["impedance_abs_distance"]
    model = ElectricalAttentionBaseline(
    ...
    return {
        ...
        "validation": {"electrical_attention": validation, "persistence_1h": persistence},
        "comparison": _compare_electrical_persistence(validation, persistence),
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        f"# Electrical-distance-biased attention: {report['config']['grid_name']} / {FEATURE_MODE}",
        "", "Validation-only metrics. Test labels and test metrics are intentionally not used.", "",
        "| Method | Scope | MAE | RMSE | WAPE (%) | sMAPE (%) |",
        "| --- | --- | ---: | ---: | ---: | ---: |",
    ]
    for method, result in (("Electrical attention", report["validation"]["electrical_attention"]), ("Persistence 1h", report["validation"]["persistence_1h"])):
        for scope in ("node_macro", "grid_aggregate"):
            values = result[scope]
    ...
    lines.extend([
        ...
        f"attention_scope: {config['attention_scope']}", f"topology_used: {config['topology_used']}",
        f"electrical_distance_used: {config['electrical_distance_used']}", f"electrical_distance_type: {config['electrical_distance_type']}",
        f"distance_normalization: {config['distance_normalization']}", f"lambda_learnable: {config['lambda_learnable']}",
        f"lambda_initial: {config['lambda_initial']:.6g}", f"lambda_final: {config['lambda_final']:.6g}",
